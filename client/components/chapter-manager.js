 ...
 *   Section C: Chapter detail view with member tables, Eintritt/Austritt
 *
 * Inject: api, apiPost, apiPut, i18n, user, isOrgaAdmin, isSuperadminAnywhere,
...
const ChapterManager = {
  ...
  methods: {
    ...
    async loadChaptersList() {
      try {
        ...
        if (!this.chSelected) {
          ...
          if (my.length === 1) this.chSelected = my[0].id;
    ...
    async chSaveAdmins(ch) {
      this.chAdminPicker.saving = true; this.chAdminPicker.error = '';
      try {
  ...
  mounted() {
    ...
    if (this.isOrgaAdmin) this.loadRequests();
  },
  template: `
<div>
  <!-- A: Overview -->
  <div v-if="!chSelected && !chEdit" class="p-6 max-w-5xl mx-auto space-y-8">
    ...
    <div v-if="isOrgaAdmin">
      ...
      <hr class="border-gray-200 mb-4" />
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        <button @click="$emit('navigate','members')"
  ...
  <div v-else-if="chEdit" class="p-6 max-w-3xl mx-auto space-y-4">
    <div class="flex items-center gap-3 mb-2">
      <button @click="chEdit = null" class="text-gray-400 hover:text-gray-700 text-sm flex items-center gap-1">← Zurück</button>
  ...
  <div v-else-if="chSelected">
    <div v-for="ch in chaptersList.filter(c => c.id === chSelected)" :key="ch.id">
      <div class="bg-white border-b border-gray-200 px-6 py-3 flex items-center gap-3">
        <button @click="chSelected = ''" class="text-gray-400 hover:text-gray-700 text-sm flex items-center gap-1.5 transition-colors">← Chapter-Übersicht</button>
        <span class="text-gray-200">/</span>
        <span class="font-semibold text-gray-800">{{ ch.name }}</span>
        <span class="text-gray-400 text-xs font-mono">{{ ch.id }}</span>
      </div>
      <div class="p-6 max-w-5xl mx-auto space-y-5">
        <!-- Hero -->
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          <div class="px-8 py-6 flex items-start justify-between gap-6">
            <div class="flex-1">
              ...
              <div class="flex flex-wrap gap-3 mt-4">
                <button @click="chSelectedSparte = null; chShowMembers = true"
                  ...
                  <div class="text-2xl font-bold text-blue-700">{{ chMbRows(ch.id).filter(r => r.status !== 'passiv').length }}</div>
                  <div class="text-left">
                    <div class="text-xs font-semibold text-blue-600">Aktive Mitglieder</div>
                    <div class="text-[10px] text-blue-400">Liste anzeigen →</div>
                  </div>
                </button>
                <button v-for="sp in ch.sparten.filter(s => !s.datumStillgelegt)" :key="sp.id"
                  @click="chSelectedSparte = sp.id; chShowMembers = false"
                  :class="chSelectedSparte === sp.id ? 'sparte-active' : 'sparte-idle'"
                  class="flex items-center gap-3 border rounded-xl px-4 py-3 transition-all group focus:outline-none">
                  <div class="text-2xl font-bold text-gray-700 group-hover:text-indigo-700">{{ chMbRows(ch.id).filter(r => r.sparte === sp.id && r.status !== 'passiv').length }}</div>
                  <div class="text-left">
                    <div class="text-xs font-semibold text-gray-600 group-hover:text-indigo-600">{{ i18n.sparte(sp.id) }}</div>
                    <div class="text-[10px] text-gray-400">Sparte →</div>
                  </div>
                </button>
              </div>
            </div>
          ...
          <div class="border-t border-gray-100 px-8 py-4 flex items-start gap-6 flex-wrap bg-gray-50/50">
            <div v-for="sp in ch.sparten.filter(s => !s.datumStillgelegt)" :key="sp.id" class="flex items-center gap-2">
              <span class="text-xs font-semibold text-gray-500">{{ i18n.sparte(sp.id) }}:</span>
              <span v-for="k in (sp.admins || [])" :key="k" class="inline-flex items-center bg-blue-50 text-blue-700 px-2 py-0.5 rounded-full text-xs">
                <span v-if="userNameCache[k]">{{ userNameCache[k] }}</span><span v-else class="font-mono">{{ k }}</span>
              </span>
              <span v-if="!sp.admins?.length" class="text-gray-300 text-xs italic">–</span>
            </div>
          </div>
        </div>

        <!-- Sparte detail -->
        <div v-if="chSelectedSparte" class="bg-white rounded-xl shadow-sm border border-blue-200 p-5 space-y-4">
          <div v-for="sp in ch.sparten.filter(s => s.id === chSelectedSparte)" :key="sp.id">
            <div class="flex items-center justify-between mb-4">
              <div class="flex items-center gap-3">
                <button @click="chSelectedSparte = null" class="text-gray-400 hover:text-gray-600 text-sm">←</button>
                <div>
                  <div class="font-bold text-gray-800">{{ i18n.sparte(sp.id) }}</div>
                  <div class="text-[11px] font-mono text-gray-400">{{ sp.id }}</div>
                </div>
                <span v-if="sp.datumStillgelegt" class="text-[10px] px-2 py-0.5 bg-orange-100 text-orange-600 rounded-full">stillgelegt {{ sp.datumStillgelegt }}</span>
              </div>
            </div>
            <div class="mb-4">
              <div class="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Spartenleiter</div>
              <div class="flex flex-wrap gap-1.5">
                <span v-for="k in (sp.admins || [])" :key="k" class="inline-flex items-center gap-1.5 bg-blue-50 text-blue-700 border border-blue-100 px-3 py-1 rounded-full text-xs font-medium">
                  <span v-if="userNameCache[k]">{{ userNameCache[k] }} <span class="opacity-50 font-mono">{{ k }}</span></span>
            ...
            <div class="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Mitglieder</div>
        ...
        <!-- Chapter admins (org-admin editable) -->
        <div v-if="isOrgaAdmin" class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 space-y-3">
          <div class="text-sm font-semibold text-gray-700">Chapter-Admins</div>
        ...
        <div v-else class="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
          <div class="text-gray-500 text-xs font-semibold uppercase tracking-wide mb-2">Chapter-Admins</div>
          <div class="flex flex-wrap gap-1.5">
            <span v-for="sa in ch.admins" :key="sa" class="bg-purple-50 text-purple-700 px-2.5 py-0.5 rounded-full inline-flex items-center gap-1 text-xs">
              <span v-if="userNameCache[sa]">{{ userNameCache[sa] }} <span class="opacity-60 font-mono text-[10px]">{{ sa }}</span></span>
              <span v-else class="font-mono">{{ sa }}</span>
            </span>
        ...
        <!-- Org-admin: read-only member list -->
        <div v-if="isOrgaAdmin && chShowMembers" class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 space-y-3">
          ...
          <div class="overflow-x-auto rounded-lg border border-gray-100">
            <table class="w-full text-xs">
              <thead class="bg-gray-50 text-gray-500 uppercase tracking-wide font-semibold">
                <tr><th class="px-3 py-2 text-left">Kürzel</th><th class="px-3 py-2 text-left">Name</th><th class="px-3 py-2 text-left">Vorname</th><th class="px-3 py-2 text-left">Sparte</th><th class="px-3 py-2 text-left">Eintritt</th><th class="px-3 py-2 text-left">Status</th></tr>
              </thead>
              <tbody>
                <tr v-if="!chMbRows(ch.id).length"><td colspan="6" class="px-3 py-8 text-center text-gray-300">Keine Mitglieder.</td></tr>
        ...
        <!-- Chapter-admin: full member management -->
        <div v-if="canManageChapterMembers(ch.id) && !isOrgaAdmin" class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 space-y-4">
          ...
          <div class="p-4 bg-green-50 border border-green-200 rounded-xl">
            <div class="text-xs font-semibold text-green-700 uppercase tracking-wide mb-3">➕ Mitglied hinzufügen (Eintritt)</div>
            <div class="flex flex-wrap gap-2 items-end">
              <div class="flex-1 min-w-36">
                <label class="text-[10px] text-gray-500 uppercase tracking-wide font-semibold block mb-1">Person suchen</label>
                <div class="relative">
              ...
              <div>
                <label class="text-[10px] text-gray-500 uppercase tracking-wide font-semibold block mb-1">Sparte</label>
              ...
              <div>
                <label class="text-[10px] text-gray-500 uppercase tracking-wide font-semibold block mb-1">Eintrittsdatum</label>
          ...
          <div v-else>
            <input v-model="chMbFilter[ch.id]" placeholder="Filtern nach Name, Kürzel oder Sportart …" class="ctrl text-xs mb-3" />
            <div class="overflow-x-auto rounded-lg border border-gray-100">
              <table class="w-full text-xs">
                <thead class="bg-gray-50 text-gray-500 uppercase tracking-wide font-semibold">
                  <tr>
                  ...
                  <tr v-for="row in chMbRows(ch.id)" :key="row.kuerzel + '|' + row.sparte"
                    ...
                    <td class="px-3 py-2">
                      <div class="flex items-center gap-1.5">
                        ...
                        <button v-if="chMbPending[ch.id + '|' + row.kuerzel + '|' + row.sparte] !== undefined"
                          @click="chMbSavePending(ch.id, row)" class="px-2 py-0.5 bg-blue-600 text-white text-xs rounded-md hover:bg-blue-700 whitespace-nowrap">✓ Speichern</button>
                      </div>
                    </td>
