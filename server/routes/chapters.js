...
const { readChapters, readChapter, readOrganisation, writeChapter, writeJson, DB_PATH } = require('../lib/gitdb');
...
router.get('/', async (req, res) => {
  ...
  const allowed = chapters.filter(c =>
    roles?.[c.id] === 'chapteradmin' || roles?.[c.id]?.role === 'spartenadmin'
  );
  res.json(allowed);
});
